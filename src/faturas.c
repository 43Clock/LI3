...
int getSalesMes(AVL *f, int mes, int tipo,int filial) {
    ...
    if (f) {
        if(filial == 0)
            ...
            r += getnumVendasFatura(f->str, filial, mes, tipo);
        return r;
    }
    return 0;
}
...
int getProductSalesaux (AVL *f, char *prod, int mes, int tipo,int filial) {
    if (f == NULL) return 0;
    if (strcmp(f->key, prod) == 0) return getSalesMes(f, mes, tipo,filial);
    else if (strcmp(f->key, prod) < 0) return getProductSalesaux(f->right, prod, mes, tipo,filial);
    else return getProductSalesaux(f->left, prod, mes, tipo,filial);
}
...
double getProfitMes(AVL *f, int mes, int tipo, int filial) {
    ...
    if (f) {
        if (filial == 0)
            ...
            r += getFaturacaoFatura(f->str, filial, mes, tipo);
        return r;
    }
    return 0;
}
...
double getProductProfitaux (AVL *f, char *prod, int mes, int tipo, int filial) {
    if (f == NULL) return 0;
    if (strcmp(f->key, prod) == 0) return getProfitMes(f, mes, tipo, filial);
    else if (strcmp(f->key, prod) < 0) return getProductProfitaux(f->right, prod, mes, tipo, filial);
    else return getProductProfitaux(f->left, prod, mes, tipo, filial);
}
